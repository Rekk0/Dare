...
**交付物**：投票屏、揭晓流程、结算页、结算写库

### 结算写库必须幂等且原子

`core/settle.ts` 已经是纯函数且验证过（10000 组 property test）。
M6 只是把它接上数据库，但**接的方式决定会不会重复发钱**：

- 整个结算在**一个事务**里：settlements 全写 + payouts 全写 + status 置 settled
- 入口走 CAS：`UPDATE activities SET status='settled' WHERE id=? AND status='voting'`，
  拿到行才继续。拿不到说明别人已经结算过了，直接返回，不报错
- 写完在**同一个事务内**断言 I2（`Σ payouts + Σ forfeited === assignments.length`），
  断言失败就抛错回滚。宁可不结算，也不能发错钱

### 揭晓的可见性必须走 visibility.ts

`projectReveal()` 已经实现且每格有测试。M6 **不许在 route handler 里另写一套过滤**，
一律调它。这是保密的唯一防线，绕过一次就等于没有。

### 结算页的叙事顺序（DESIGN.md §6.4）

不要一次性铺开，按顺序讲：
1. 逐个揭晓每人的任务和结果（可左右滑）
2. 识破关系图，连线一条条画出来
3. 份额榜，每人从 0 计数上去
4. 底部一行：共 N 份 / 发出 X / 作废 Y

被识破的人在名字后挂红色 `已暴露` 标签，**数字本身转灰不用红** ：
红只留给被识破的那一刻。
...
- [ ] **并发结算测试**：同一活动并发跑 5 次结算，payouts 只有一套
- [ ] **事务回滚测试**：故意构造一个 I2 不成立的场景，断言事务回滚且
      活动状态仍是 voting（宁可不结算也不发错钱）
- [ ] 揭晓接口的响应经过 `projectReveal()`，`settled` 前拿不到识破者身份
