...
本项目采用 **[GNU AGPL v3](LICENSE)**。

用大白话说：

- **可以商用。** 拿去开公司、拿去收费，都行，不用问我。
- **改了就得开源。** 你基于它做的修改版本，必须同样用 AGPL v3 放出源码。
- **架成网站也算。** 这是 AGPL 和 GPL 的唯一区别，也是这里选它的原因：
  只要有人通过网络使用你那个改过的版本，你就得向这些人提供对应的源码。
  自己关起门来改着玩不受影响，一旦对外提供服务就要开源。

**部署者注意：** AGPL 第 13 条要求你向使用者提供获取源码的途径。
自部署时请在界面上留一个指向你那份源码的链接。
