...
| A3 | 参与者 3-21 人，同一物理场景，活动时长 1-6 小时 | 所有算法可用暴力解 | 上限从 12 提到 21，猜测配额随之改成按人数算，见 §5.8 |
...
activities(
  ...
  guess_quota   INT     DEFAULT 3,   -- 每人全场猜测总次数。分配时按人数覆写，见 §5.8
...
- `feasibility`、`stealth`、`verifiability` 任一低于**本局设定的下限** -> `revise`，展示建议，但**允许用户坚持提交**（AI 是顾问不是法官，跟公投机制一致）

**预审标准是每局自己定的。** 同事团建和死党局的尺度本来就不该一样，
所以三个下限和一个尺度档位存在 `activities` 表上，建活动时由创建者设定：

| 字段 | 默认 | 含义 |
|---|---|---|
| `min_feasibility` | 30 | 可完成度下限 |
| `min_stealth` | 20 | 隐蔽度下限。隐蔽是核心机制，一眼被看穿的任务毁掉整局 |
| `min_verifiability` | 20 | 可留证下限 |
| `edginess` | 2 | 尺度档位：1 温和 / 2 常规 / 3 大胆 |

建活动页有个「让 AI 看看这个场景」按钮，调 `POST /api/activities/suggest-policy`
拿一组建议值填进表单。**这只是填默认值，创建者随时可以改。**

**AI 是顾问，不是审查员。** 出题的自由留给玩家，预审只负责打分和提醒，
能不能上由攒局的人看着四个分数自己决定。默认放行。

只有三类判 `safety=block`，也就是模型自己都不该生成的内容：

1. 触犯法律的行为
2. 极端暴力，会造成严重人身伤害
3. 色情内容或性骚扰

**除此之外一律不拒。** 下面这些都是打分范畴：违反场所规定、参与者之间自愿的轻度身体冲击、
藏不住（压 stealth 分）、让人尴尬（这正是这个游戏好玩的地方）、超出本局尺度（给 warn）。

**只评估任务写了什么，不推测它可能会怎样。** 任务写「让一个人」「让某位异性」时
一律指本局参与者；只有白纸黑字要求找「陌生人」「路人」「服务员」「邻桌」时，
才给 `warn` 并写进 reasons 第一条。
出题页会把这条理由用红边框显示出来 - 不显示等于这个提醒白给。

**出题分两步，只有第二步写库。**

预审只是问 AI 一句意见，用户还没决定要不要交，**这一步不产生任何持久化**。
它返回评分和一张 HMAC 预审凭据（`core/review-token.ts`），凭据按住
「哪个活动、哪个人、哪段正文」，评分也装在里面。

确认时带上正文和凭据，验签通过才入库。**判成 reject 的题不发凭据**，交不上去。
这样安全线靠签名守，不靠往库里写一行草稿：换正文验不过，
「拿正常题过预审、确认时换成被拦的内容」这条路是堵死的。
入库的评分从凭据里解出来，不取请求体 - 客户端说了算的数字不该进库。

确认不重跑预审：既省 2 到 5 秒，也避免模型随机性给出跟用户刚看到的不一样的分数。
凭据 15 分钟过期。改了题目文字之后评分卡收起、确认按钮禁用，必须重新预审。
这是刻意的取舍：拦下来更安全，但会误伤大量正常的题，
所以改成把判断权交回攒局的人。

**提示词里要显式写「这局里的人都是自愿参加的」。**
...
- **配额是全场的，不是每目标的**。这封死了「出题者拿自己的题去扫全场 N-1 个人」的漏洞
- **配额随人数走**：不足 9 人给 3 次，9 人及以上按 `floor(人数 / 3)`。固定 3 次在人多时会让猜测变成摆设（20 个目标里猜 3 次几乎猜不中），而被识破是这个游戏最好的情绪节点。实现见 `core/lifecycle.ts` 的 `guessQuotaFor()`，分配时写进 `activities.guess_quota`
...
| 后端 | Next.js Route Handlers（同仓） | N ≤ 21、并发极低，不需要独立后端服务 |
...
5. **识破率是多少？** - v0.3 下被猜中直接归零，识破率过高会导致大面积没人拿钱、体验挫败。`guess_threshold`（默认 75）和 `guess_quota`（由 `guessQuotaFor()` 按人数算）就是调这个的两个旋钮
